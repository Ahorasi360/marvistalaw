...
async function getRows(offset) {
  const url = SB + '/rest/v1/marvistalaw_pages?select=id,content,service_name,content_es&content_es=is.null&content=not.is.null&limit=5&offset=' + offset;
...
export async function GET(request) {
  try {
    ...
    return NextResponse.json({ done: !more, offset, ok, errors: err.length, err_sample: err.slice(0, 2), next: offset + 5 });
  } catch (e) {
